...
class CreateDiscussionPage extends Component {
	...
	render() {
		...
		return (
			(this.props.hasSubmitted && hasSubmitted ? <Redirect to={`/discussions/${this.props.id}`} /> : 
				<Provider store={store}>
					<div className='createDiscussionPage'>
						<DisplayMetaTags page='createDiscussion' props={this.props} state={this.state} />
						<PageHeader
							{...this.props}
						/>
						<Container 
							...
							<Grid>
								...
								<Grid.Column className='rightSide' width={4}>
									...
									<List relaxed>
										...
										<List.Item>Why be wrong about something for a second longer than you have to?</List.Item>
										<List.Item>Recognize that your entire sense of identity shouldn't be wrapped up in your beliefs</List.Item>
									</List>
								</Grid.Column>
							</Grid>
						</Container>
